from flask import Flask, render_template, request, redirect, url_for, flash, session, send_from_directory, send_file, jsonify, Response, stream_with_context
...
db_path = os.path.join(os.path.dirname(basedir), 'crime_report.db')
...
def home():
    ...
    # For anonymous users, show limited verified crimes by location and verification status
    else:
        # Get verified crimes from the last 30 days, limited to 5
        recent_crimes = CrimeReport.query.filter(
            CrimeReport.is_verified == True,
            CrimeReport.timestamp >= datetime.now() - timedelta(days=30)
        ).order_by(CrimeReport.timestamp.desc()).limit(5).all()
    ...
    if request.method == 'POST':
        try:
            ...
            if evidence and evidence.filename:
                ...
                evidence_filename = f"evidence_{int(time.time())}_{filename}"
            ...
            if suspect_sketch_data:
                sketch_filename = f"sketch_{int(time.time())}.png"
                sketch_path = os.path.join(app.config['UPLOAD_FOLDER'], sketch_filename)
                with open(sketch_path, 'wb') as f:
                    f.write(base64.b64decode(suspect_sketch_data.split(',')[1]))
...
def admin_dashboard():
    ...
    # Filter reports based on admin's jurisdiction if specified
    if current_user.jurisdiction:
        reports = [report for report in reports if current_user.jurisdiction.lower() in report.location.lower()]
    
    # Get statistics for filtered reports
    stats = get_crime_statistics()
    
    # Get unique locations from filtered reports for filtering
    ...
    return render_template('admin_dashboard.html', 
                          ...
                          admin_jurisdiction=current_user.jurisdiction)
...
def update_report(report_id):
    ...
    # Check if admin has jurisdiction over this report
    if current_user.jurisdiction and current_user.jurisdiction.lower() not in report.location.lower():
        flash('You do not have jurisdiction over this report.', 'danger')
        return redirect(url_for('admin_dashboard'))
    ...
    if request.method == 'POST':
        try:
            ...
            location = request.form.get('location', report.location)
            status = request.form.get('status', report.status)
            
            # Only update lat/long if they are provided and valid
            try:
                latitude = float(request.form.get('latitude', report.latitude))
                longitude = float(request.form.get('longitude', report.longitude))
            ...
            # Handle AJAX request
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({'success': False, 'message': f'Error updating report: {str(e)}'}), 400
...
def verify_report(report_id):
    ...
    if not notes:
        flash('Please add verification notes.', 'danger')
        return redirect(url_for('admin_dashboard'))
    ...
    if action == 'verify':
        ...
        report.verified_by = current_user.id
        report.verification_notes = notes
        flash('Report marked as fake.', 'warning')
...
def sos():
    ...
    if request.method == 'POST':
        ...
        if action == 'add_contact' and current_user.is_authenticated:
...
def notify_police_about_sos(sos_alert):
    """Send notifications to police officers about a new SOS alert"""
    try:
        # Find police officers in the area
        nearby_police = User.query.filter(
            User.is_police == True,
            User.jurisdiction.ilike(f"%{sos_alert.location}%") if sos_alert.location else True
        ).all()
        ...
        for officer in nearby_police:
            if officer.email:
                msg = Message(
                    ...
                    'UPDATE: SOS Alert Cancelled',
                    recipients=[officer.email]
                )
                msg.body = f'''
                SOS ALERT CANCELLED
                
                User: {current_user.username if current_user.is_authenticated else "Anonymous User"}
                Time: {sos_alert.timestamp.strftime('%Y-%m-%d %H:%M:%S')}
                Location: {sos_alert.location if sos_alert.location else 'Unknown'}
                Coordinates: {sos_alert.latitude}, {sos_alert.longitude}
                
                This SOS alert has been cancelled by the user.
                '''
                mail.send(msg)
    except Exception as e:
        print(f"Error notifying police about cancellation: {str(e)}")
...
@app.route('/check_sos_status/<int:sos_id>')
def check_sos_status(sos_id):
    """Check if an SOS alert is still active - for anonymous users"""
    sos_alert = SOSAlert.query.get_or_404(sos_id)
    
    # Security check - only allow checking anonymous SOS alerts or user's own alerts
    if current_user.is_authenticated:
        if sos_alert.user_id != current_user.id and not current_user.is_police and not current_user.is_admin:
            return jsonify({"active": False, "error": "Not authorized"})
    else:
        # For anonymous users, only let them check anonymous alerts
        if not sos_alert.is_anonymous:
            return jsonify({"active": False, "error": "Not authorized"})
    
    return jsonify({
        "active": sos_alert.status == 'active',
...
def trigger_sos():
    try:
        ...
        # Try to notify police, but don't stop if it fails
        try:
            notify_police_about_sos(sos_alert)
        except Exception as e:
            print(f"Error in police notification: {str(e)}")
...
def cancel_sos(sos_id):
    ...
    # Check if the user is authorized to cancel this SOS alert
    if current_user.is_authenticated:
        if sos_alert.user_id != current_user.id and not current_user.is_police and not current_user.is_admin:
            flash('You are not authorized to cancel this SOS alert.', 'danger')
            return redirect(url_for('sos'))
    else:
        # For anonymous users, we need to verify they are the one who created the alert
        # This could be done with a token or session ID, but for simplicity, we'll just check if the alert is anonymous
        if not sos_alert.is_anonymous:
            flash('You are not authorized to cancel this SOS alert.', 'danger')
            return redirect(url_for('sos'))
...
@app.route('/clear_all_sos', methods=['GET', 'POST'])
@login_required
def clear_all_sos():
    """Clear all SOS alerts from the system"""
    if not current_user.is_police and not current_user.is_admin:
        flash('You do not have permission to perform this action.', 'danger')
        return redirect(url_for('home'))
    
    try:
        # Delete all SOS alerts
        num_deleted = SOSAlert.query.delete()
        db.session.commit()
        flash(f'Successfully removed {num_deleted} SOS alerts from the system.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error removing SOS alerts: {str(e)}', 'danger')
    
    return redirect(url_for('police_sos_alerts'))

def generate_report_updates():
    """Generate SSE data for report updates"""
    last_check = time.time()
    
    while True:
        # Get new reports since last check
        new_reports = CrimeReport.query.filter(
            CrimeReport.timestamp >= datetime.fromtimestamp(last_check)
        ).all()
        ...
        if new_reports:
            ...
            # Send the data
            yield f"data: {json.dumps(reports_data)}\n\n"
            
        last_check = time.time()
        time.sleep(2)  # Check every 2 seconds

@app.route('/stream-reports')
@login_required
def stream_reports():
    """SSE endpoint for streaming report updates"""
    if not current_user.is_police and not current_user.is_admin:
        return "Unauthorized", 403
        
    return Response(
        stream_with_context(generate_report_updates()),
...
@login_required
def get_statistics():
    """Get updated statistics for the dashboard"""
    if not current_user.is_police and not current_user.is_admin:
        return jsonify({'error': 'Unauthorized'}), 403
    ...
    try:
        ...
        stats['pending'] = CrimeReport.query.filter_by(status='pending').count()
...
if __name__ == '__main__':
    ...
    app.config['SESSION_COOKIE_DOMAIN'] = False  # or set to your IP if needed
